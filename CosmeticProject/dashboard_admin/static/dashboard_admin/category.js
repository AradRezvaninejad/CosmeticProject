...
create_category.addEventListener("click",()=>{
    if (category_name.value && category_slug.value && description.value){
        ...
        post_("http://127.0.0.1:8000/dashboard-admin/categories",{"work":"add_cat","description":description.value,"slug":category_slug.value,"name":category_name.value}).then(result=>{
            if (result.ok){
